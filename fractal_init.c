...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/03/14 15:33:33 by chorange          #+#    #+#             */
/*   Updated: 2019/03/14 16:12:47 by chorange         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		transform_init(t_fractal *fractal)
{
	if (fractal->fractal_type == mandelbrot)
		mandelbrot_init(&fractal->transform);
	else if (fractal->fractal_type == julia)
		julia_init(&fractal->transform);
	else if (fractal->fractal_type == irina)
		irina_init(&fractal->transform);
	else if (fractal->fractal_type == ship)
		ship_init(&fractal->transform);
	else if (fractal->fractal_type == spider)
		spider_init(&fractal->transform);
	else if (fractal->fractal_type == branches)
		branches_init(&fractal->transform);
