__kernel void branches(__global char *image_data, __global t_transform *transform)
{
    int y = get_global_id(0) / WIDTH;
    int x = get_global_id(0) % WIDTH;
    double newRe, newIm, oldRe, oldIm;

    newRe = 0.5 * (x - WIDTH / 2) / (0.5 * transform->zoom * WIDTH) + 0.5 * transform->delta_x;
    ...
    while (i++ < transform->max_iterations)
    {
        oldRe = newRe;
        oldIm = newIm;
        newRe = oldRe * oldRe - oldIm * oldIm - 1.978942 * oldRe * oldIm;
        newIm = 3 * oldRe * oldIm + 0.617822;
